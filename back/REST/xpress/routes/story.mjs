...
router.post("/",(req,res)=>{
  ...
  const tagQuery = tags.map((x)=>`merge (${x.replace("#","")}:Tag {name: ${pesos}${x.replace("#","")}})  create (s)-[:Is]->(${x.replace("#","")}) `).join(" ");
  let tagDict = {};
  for(let i of tags){
    tagDict[i.replace("#","")] = i;
  }
  ...
  // console.log(tagDict);
  // console.log(myQuery);
  query(myQuery+tagQuery, tagDict, "row", () => {
...
router.get("/:id", (req, res) => {
  query("match (s:Story) match(a:Author) match (t:Tag) where id(s) = $sid and (a)-[:Wrote]->(s) and (s)-[:Is]->(t) return s,a,collect(t),id(s)", {
