...
def updateAuthor():
    incoming = request.json
    query("match (n:Author) where n.username = $uname set n = $props",
          {"props": incoming, "uname": incoming["username"]})
    return getAuthor(incoming["username"])
...
def getAuthorStories(author):
    resp = query("match (a:Author)-[:Wrote]->(s:Story) where a.username = $uname return s.title, s.date, id(s)",
                 {"uname": author}, returnType="row")
    return Response(json.dumps(resp), content_type='application/json')
